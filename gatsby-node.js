...
exports.sourceNodes = async (args, options) => {
  ...
  for (
    let contentType, TypeNode, types, typeConfig, ct = 0;
    ct < contentTypes.length;
    ct++
  ) {
    ({ type: contentType, multiple } = contentTypes[ct]);

    typeConfig = config.contents.find(({ type }) => type === contentType);
    types = await listContentType(
      cockpit,
      contentType,
      typeConfig.whitelist,
      typeConfig.blacklist
    );
    ...
    for (let type, spec, content, parent, Node, t = 0; t < types.length; t++) {
      ...
      spec = spec.fields
        .map((f) => [f.name, getFieldSpecification(f)])
        .reduce((acc, [name, s]) => ({ ...acc, [name]: s }), {});
        ...
        const getNodesFromEntry = async (
          ...
          if (cacheData && modified === cacheData.modified) {
            ...
            await cache.set(cacheKey, {
              modified,
              nodes: [processed.node.id].concat(
                processed.nodes.map((n) => n.id)
              ),
            });
          }
...
const fetchAssets = async (config, assetMap, cockpit, args) => {
  ...
  for (
    let asset, cacheKey, cacheData, modified, url, file, a = 0;
    a < assets.length;
    a++
  ) {
    ...
    if (!nodeId) {
      url = cockpit.getApiUrl(
        `${trim(uploadPath, "/")}/${trim(asset.path, "/")}`
      );
...
const processEntry = (
  ...
  // console.log('specxx', specifications)
...
const isAssetDownloadEligible = (asset, filetypes, tags) => {
  const suitableFiletype = Object.entries(filetypes)
    .map(([key, value]) => value === true && asset[key] === true)
    .includes(true);
